...
* **A cleanup race that Linux always won.** `ShutdownAndStopAll` returns once it has *signalled* each process, not once they have exited. Those children hold log files open under the registry directory, so a `t.TempDir()` registry can be removed while a dying writer still holds it — `TempDir RemoveAll cleanup: directory not empty`. Locally on Linux the race was won every time, including under `-count=3`; macOS lost it consistently. Fixed with a `stopAndWait` helper that blocks until the PIDs are actually gone via the existing `IsPIDAlive`. *Rule: "passed `-count=3` locally" is evidence about one platform's scheduler, not proof that a teardown is ordered correctly.*
