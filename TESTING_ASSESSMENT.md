...
**4. Two macOS-only test failures that every local Linux run passed.** Both were caught by the CI matrix, not by local verification, and both are worth internalising because the same shape will recur:

* **Hardcoded page size.** The `parseStatmRSS` test computed its expectations as `250 * 4096` with a comment noting 4096 "matches `os.Getpagesize()` on virtually all Linux targets". Apple Silicon uses **16384**, so the test failed on both macOS legs (`= 4096000, want 1024000`) while passing everywhere locally. The production code was correct throughout — it always called `os.Getpagesize()`. The test now derives from the same call. *Rule: a test asserting on a platform constant must obtain it the same way production does, never restate it as a literal.*
* **A cleanup race that Linux always won — and that took two attempts to diagnose.** Symptom: `TempDir RemoveAll cleanup: directory not empty` on the registry directory.

  The *first* diagnosis was that `ShutdownAndStopAll` returns once it has signalled each process rather than once they have exited, so a dying writer could still hold a file. A `stopAndWait` helper was added to block on `IsPIDAlive` until the captured PIDs were gone. **That was incomplete, and the next CI run failed on all four legs instead of two** — Ubuntu included.

  The actual cause is upstream of process death. `runProcessLoop` checks `stopChan` only at the top of its loop, so a `Stop()` landing just after that check still permits one more iteration, which **creates a fresh log file in the registry directory after `Stop` has returned**. Waiting on PIDs cannot help: under `RestartAlways` with a millisecond backoff the captured PID is frequently already dead between respawns, so `stopAndWait` returned immediately while the loop went on to create another file. `t.TempDir()`'s `RemoveAll` is not retried, so losing that race fails the whole test.

  Fixed with a `newRegistryDir(t)` helper that allocates the registry via `os.MkdirTemp` and removes it with bounded retries, registered before the shutdown cleanup so `t.Cleanup`'s LIFO ordering stops the processes first. `stopAndWait` is retained — it is still what prevents orphaned daemons — but it is not what fixes this.

  *Two rules.* "Passed `-count=3` locally" is evidence about one platform's scheduler, not proof that a teardown is ordered correctly; 12 local runs could not reproduce what CI hit immediately. And a plausible diagnosis that makes a failure *worse* is a signal the causal chain is longer than assumed — the fix should have been validated against the mechanism, not just against a green local run.

  *Latent production wart this exposed, not fixed here:* because the `stopChan` check is not atomic with the spawn, `Stop()` can leave one freshly-spawned detached process running that it never terminates, since it only signals the PID it read on entry. Harmless for the TUI's own shutdown path, but it means `Stop` is not strictly authoritative. Worth a follow-up.

This is the concrete payoff of the macOS matrix leg that [§3.1](#31-ci-workflow-review-githubworkflowsciyml) lists as a strength — it caught two defects that Linux-only verification, including deliberate repeat runs, could not.
